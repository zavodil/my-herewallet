...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const connectorApprove = document.body.querySelector<HTMLAnchorElement>(".here-connector-approve")!;
  const closeButton = document.body.querySelector<HTMLDivElement>(".here-connector-close-button")!;
  const dontWallet = document.body.querySelector<HTMLButtonElement>(".here-connector-text")!;

  let isApproving = false;
  let requestId = "";

  const tooggleAppClip = () => {
    ...
    const link = useAppClip ? `https://h4n.app/${requestId}` : `herewallet://h4n.app/${requestId}`;
  ...
  dontWallet.addEventListener("click", () => {
    const useAppClip = !(localStorage.getItem("useAppClip") === "true");
    localStorage.setItem("useAppClip", useAppClip ? "true" : "false");
    tooggleAppClip();
  });
